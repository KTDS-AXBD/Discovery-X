...
- ✅ 문서 레지스트리: `app/lib/docs/registry.ts` — Vite `?raw` 빌드타임 임포트, 카테고리 3종 (기획/운영/가이드)
- ✅ MarkdownViewer 컴포넌트: ReactMarkdown + prose + Axis 토큰 (MessageBubble 패턴 재사용)
- ✅ DocsSidebar 컴포넌트: 카테고리별 그룹 + 활성 하이라이트
- ✅ 반응형: Desktop lg+ 사이드바 / Mobile select 드롭다운
- ✅ MainNav에 Docs 링크 추가 (Radar와 Settings 사이)
