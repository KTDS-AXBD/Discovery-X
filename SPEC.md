...
### 현재 단계
**🚀 v4.1 UX 리팩토링 — 완전 한국어화 진행 중 (2026-02-04)**
...
- 남은 작업: WU-F (Group 2 잔여) → WU-H/I (Group 3)

### 이전 변경 (세션 104 후반)
**UX 리팩토링 v4.1 — WU-C/D/E 완료 + 인프라 보강**:
- ✅ WU-C: Dashboard 탭명 한국어화 (파이프라인/지표/건강도/알림/활동 기록)
- ✅ WU-D: MainNav 메뉴명 한국어화 (현황판/사업 탐색/방법론/맥락 그래프/레이더/문서/설정/관리)
- ✅ WU-E: guide-messages 용어 교체 — 48개 메시지 + 8개 상태 헤더 + CTA 라벨 전체 한국어화
- ✅ Daily Cron 강화: Stage SLA 14일 체류 알림, ACTIVE_STATUSES 상수화, 반환 타입 수정
- ✅ Weekly Summary Cron 엔드포인트 추가 (api.cron.weekly-summary.ts)
- ✅ Embeddings 스키마 + 서비스 추가 (drizzle 0013_embeddings.sql)
- ✅ Gate timeout 테스트, weekly-summary 테스트, active-statuses 테스트 추가
- ✅ 빌드 검증 통과 (typecheck + lint + build)
- 남은 작업: WU-F/G (Group 2 일부) → WU-H/I (Group 3)
...
| # | 항목 | Phase | 상태 | 파일 수 |
|---|------|-------|------|---------|
| F1 | UX 리팩토링: 용어 완전 한국어화 | v4.1 P1 | 🔄 | 7 |
| F2 | UX 리팩토링: 가이드 메시지 보강 | v4.1 P2 | ⬜ | 2 |
| F3 | UX 리팩토링: EmptyState 범용화 | v4.1 P3 | ⬜ | 4 |
| F4 | UX 리팩토링: 메뉴/진입점 개선 | v4.1 P4 | ⬜ | 2 |
