...
테스트 커버리지 확장 (Phase 1+2): 테스트 DB 마이그레이션 0007~0010 현행화, v3 fixture 헬퍼 11개 추가, DB 스모크 테스트 15개, Agent discovery-tools 통합 테스트 52개 → 전체 196개 통과.
테스트 커버리지 확장 (Phase 3): Agent method-tools 통합 테스트 20개 추가 → 전체 216개 통과.
문서 현행화: CLAUDE.md v3 반영 (11단계 파이프라인, 52개 라우트 구조, @axis-ds 스택), SPEC.md §5/§6 세션 69-72 반영. 프로덕션 배포 (1741efa4).
테스트 커버리지 확장 (Phase 4): Agent ontology-tools 통합 테스트 21개 추가 → 전체 237개 통과.
문서 및 폴더 정비: SPEC.md §5 이전 변경 620행 축약, §6 완료된 남은/미래 작업 섹션 삭제, 로컬 아티팩트 파일 4건 삭제. 프로덕션 배포 완료 (42222cdd).
...
- ✅ `query-tools.ts`: `getExperimentContext` 함수 추가 (~140줄)
  - 실험 슬롯 현황 (used/max/canAdd)
  - Method Run 결과 + Method Pack 정보 (nameKo, tier 포함)
  - 미검증 assumptions 목록
  - 근거 요약 (강도별 분포, 강한 근거 목록)
  - 추천 (실험 포커스 제안, 미실행 Tier-0 팩)
- ✅ `tool-registry.ts`: TOOL_MIN_AUTONOMY (Level 1) + AGENT_TOOLS 등록
- ✅ `executor.ts`: switch case 추가
...
| **테스트 인프라** | ✅ | Vitest + Playwright, 전체 252개 통과 (unit 76 + DB 스모크 15 + Agent 도구 통합 108 + 기존 integration 53) |
