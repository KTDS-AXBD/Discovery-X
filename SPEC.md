...
### 최근 변경 (세션 29)
**디자인 토큰 컴포넌트 마이그레이션 확대 — 6개 라우트 전환**:
- ✅ `discoveries.$id.tsx` — PageLayout, Card, Button, Badge, AlertBanner, Select, cn() 적용
- ✅ `review.tsx` — 디자인 토큰 컴포넌트 전환
- ✅ `recall.tsx` — 디자인 토큰 컴포넌트 전환
...
- **DB 마이그레이션**: ✅ 5개 마이그레이션 (0000~0004) 프로덕션 적용 완료
- **빌드 상태**: `pnpm build` (345KB server) + `pnpm typecheck` + `pnpm lint` 모두 통과
- **Radar Worker**: ✅ 프로덕션 배포 완료 (https://radar-worker.sinclair-account.workers.dev), Cron 매일 9:00 KST, RSS 5소스 활성
- **배포 상태**: ✅ 세션 27 프로덕션 배포 완료 (https://dx.minu.best / https://discovery-x.pages.dev)
...
**Design Token 마이그레이션 잔여 (선택)**
- [ ] 나머지 라우트 페이지 (상세, 편집, 승격, 결정, review, recall, metrics 등) 새 컴포넌트 시스템으로 전환

**Radar 확장 (선택)**
- [ ] Web/YouTube 소스 추가 (Product Hunt, Hugging Face, Y Combinator, Two Minute Papers)
- [ ] GeekNews RSS 대안 확인 (현재 403 차단)
