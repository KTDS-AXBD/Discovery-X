...
**v3 R3b (다음 세션)**
- [ ] 알림 엔진: alert_rules → cron 연동 → alerts 생성 (KPI 임계치, 단계 SLA, 기한 초과)
- [ ] 웹훅 커넥터: webhook_configs → 이벤트 발생 시 외부 전송 (Slack/Teams)
- [ ] 역할 기반 권한: Gatekeeper 역할 + 감사 로그
- [ ] UI 폴리시: KPI 카드, Discovery 링크 시각화, Gate 승인 UI
