...
### 현재 단계
**🚀 v4.2 Embeddings 인프라 완료 + 채팅 UX 개선 (2026-02-04)**
...
### 최근 변경 (세션 109)
**F4/F5 Embeddings 인프라 완료 + 채팅 UX 개선**:
- ✅ F4: Vectorize 인덱스 2개 생성 확인 (dx-discovery-embeddings, dx-evidence-embeddings)
- ✅ F4: DB 마이그레이션 0013_embeddings.sql 프로덕션 적용 확인
- ✅ F4: OPENAI_API_KEY 프로덕션 업데이트 + 재배포 성공
- ✅ F4: 초기 임베딩 전량 동기화 (Discovery 33개 + Evidence 1개)
- ✅ F5: cron-job.org에 "DX Embeddings Sync" 등록 (*/15 * * * *, batch=10)
- ✅ F5: 테스트 실행 200 OK 확인 (1.41s 응답)
- ✅ 채팅: SUGGESTIONS 주석 파싱 → 동적 후속 질문 칩 UI (ChatPanel)
- ✅ 채팅: 테이블/헤딩/코드블록/인용 커스텀 마크다운 스타일 (MessageBubble)
- ✅ 채팅: ToolExecution 리팩토링 (AlertBanner→cn, 카테고리별 스타일, 쿼리도구 확장)
- ✅ Agent: 후속 질문 제안 지시문 추가 (system-prompt.ts)
