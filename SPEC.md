...
### 현재 단계
**🚀 운영 실험 진행 중 (2026-01-31 시작)**

P0 전 항목 구현 + QA 검증 완료. 프로덕션 DB에 사용자 5명 등록, 운영 실험 시작. 30-60일간 Discovery 5-10건 목표.
...
- ✅ `radar-worker/` — 별도 Cloudflare Worker 프로젝트
  - RSS/Web/YouTube 수집기 (RSS XML 파싱 + Atom 지원)
  - SHA256 URL + FTS5 제목 유사도 2중 중복 제거
  - GPT-4o-mini 관련성 점수(0-100) + 한국어 번역
  - 자동 INBOX Seed 생성 파이프라인 (threshold 60+, 최대 5개/run)
  - Cron Trigger 매일 0:00 UTC (9:00 KST) + HTTP 수동 트리거
...
- **DB 마이그레이션**: ✅ 5개 마이그레이션 (0000~0004), 0004 로컬 적용 완료 (프로덕션 미적용)
- **빌드 상태**: `pnpm build` (336KB server) + `pnpm typecheck` + `pnpm lint` 모두 통과
- **Radar Worker**: 🆕 `radar-worker/` 별도 CF Worker 프로젝트 (배포 전, `pnpm install` + secrets 설정 필요)
- **배포 상태**: ✅ 세션 23 프로덕션 배포 완료 (https://dx.minu.best / https://discovery-x.pages.dev)
...
| **Radar 자동 토픽 수집** | ✅ | DB 스키마 + UI + radar-worker (배포 전) |

### 남은 작업
- [x] 최종 프로덕션 배포 — 세션 14에서 완료
- [x] Resend secrets 설정 + 외부 cron 서비스 연동 — 세션 23에서 완료
- [ ] Radar 프로덕션 배포 — `pnpm db:migrate:prod` + `radar-worker` 배포 + 소스 시딩
...
**Radar Phase 3 (기본 소스 시딩)**
- [ ] 기본 RSS 소스 5개 등록 (GeekNews, TechCrunch AI, HN Best, MIT Tech Review, arXiv cs.LG)
- [ ] Web/YouTube 소스 추가 (Product Hunt, Hugging Face, Y Combinator, Two Minute Papers)
