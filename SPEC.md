...
### 현재 단계
**🚀 v4.3 lilys.ai 참고 UI/UX 개선 — Phase 1+2 전체 완료 (세션 110, 2026-02-04)**

v3 R0~R3b 완료 후, v4 Venture Discovery Sprint MVP 구현 및 프로덕션 배포 완료.
- ✅ vd_* 16개 테이블 스키마 완성 + 프로덕션 마이그레이션 적용 (0011_venture_sprint.sql, 0012_task_queue_dedupe.sql)
- ✅ 도메인 모듈 전체 구현 완료 (types/constants/schemas/domain/repositories/ui)
- ✅ `/venture/*` 라우트 18개 구현 완료 (페이지 13개 + API 5개)
- ✅ UI 컴포넌트 13개 구현 완료 (EmptyState/OnboardingGuide 추가)
- ✅ 프로덕션 배포 완료 (https://dx.minu.best)
- ✅ 프로덕션 스프린트 생성 기능 검증 완료
- ✅ MainNav에 Venture 링크 추가 완료
- ✅ venture-worker 구현 완료 (D1 폴링 큐 기반, 8개 핸들러)
- ✅ Task Queue Retry/Backoff/Idempotency Gap 보완 완료
- ✅ Task Executor 시스템 구현 완료 (8개 타입 + 워커 API + 수동 트리거)
- ✅ venture-worker 전체 핸들러 테스트 완료 (8개 핸들러 모두 정상 동작 확인)
- ✅ Deep Dive 탭 Action 구현 완료 (Assumption/Premortem/LeanCanvas CRUD + AI 분석 트리거)
- ✅ Packaging 탭 Action 구현 완료 (Artifact CRUD + AI 문서 생성 트리거 + 스프린트 완료)
- ✅ 테스트 커버리지 대폭 개선: scoring-policy 100%, task-queue 98%+, sprint-state-machine 100%, sprint-repository 100%
- ✅ 온보딩 UX 개선: EmptyState + OnboardingGuide 컴포넌트 구현
- ✅ Gate 2 → Packaging → 스프린트 완료 E2E 테스트 완료
- ✅ venture-worker E2E 파이프라인 테스트 완료 (Task API 수동 트리거)
- ✅ Sprint Repository 통합 테스트 36개 추가 (13개 함수 100% 커버리지)
- ✅ Markdown Export 기능 구현 완료 (Lean Canvas/Pitch Deck/One Pager/Executive Summary 변환)
- ✅ Task 의존성 기반 순서 보장 구현 완료 (세션 102)
- ✅ Gate 승인 SLA Timeout 자동 처리 + 리마인더 구현 (세션 104)
- ✅ UX 리팩토링 WU-A/WU-B 완료: 상수/탭명/퍼널/Gate 결정 한국어화 (세션 104)
- ✅ UX 리팩토링 Group 1 완료: WU-C/D/E/G 추가 완료 (세션 104~105)
- ✅ UX 리팩토링 WU-C/D/E 완료: Dashboard 탭/MainNav 메뉴/guide-messages 한국어화 (세션 104)
- ✅ Daily Cron 강화: Stage SLA 체크(14일), ACTIVE_STATUSES 상수, Weekly Summary 엔드포인트
- ✅ Embeddings 스키마 + 서비스 추가 (drizzle 0013)
- ✅ Vectorize 인덱스 2개 생성 + 프로덕션 배포 (F4)
- ✅ Embeddings Cron 15분 간격 외부 스케줄 등록 (F5, cron-job.org)
- ✅ 초기 임베딩 전량 동기화 완료 (Discovery 33개 + Evidence 1개)
- ✅ 채팅 후속 제안 칩 UI + 마크다운 렌더링 개선 + ToolExecution 리팩토링
- ✅ ContextPanel 사이드패널 + generate_discovery_digest 도구 (44번째 Agent 도구)
- ✅ 리치 도구 결과 시각화: PipelineFlow, EvidenceChart, EvidenceCard 컴포넌트
- ✅ ToolExecution 43개 도구 전체 레이블 매핑
- ✅ prose 다크모드 토큰 추가 (--dx-code-bg, blockquote)
- ✅ 프로덕션 배포 완료 (e2cea46f)

### UX 리팩토링 로드맵 (v4.1)

| WU | 작업 단위 | 파일 수 | 의존성 | 상태 |
|----|-----------|---------|--------|------|
| WU-A | 상수 한국어화 (상태/결정/퍼널) | 4 | 없음 | ✅ |
| WU-B | Venture 탭명 한국어화 | 1 | 없음 | ✅ |
| WU-C | Dashboard 탭명 한국어화 | 1 | 없음 | ✅ |
| WU-D | MainNav 메뉴명 한국어화 | 1 | 없음 | ✅ |
| WU-E | guide-messages 용어 교체 + 보강 | 1 | WU-A, WU-B | ✅ |
| WU-F | NextStepGuide currentTab 검증 | 1 | WU-B | ✅ |
| WU-G | EmptyState 범용화 | 1 | 없음 | ✅ |
| WU-H | 탭별 EmptyState 적용 (inbox/longlist/gate) | 3 | WU-G, WU-E | ✅ |
| WU-I | 홈 화면 진입점 안내 | 1 | WU-D | ✅ |

병렬 실행 그룹:
- **Group 1** (독립): ✅ WU-A + WU-B + WU-C + WU-D + WU-G — 전체 완료
- **Group 2** (Group 1 이후): ✅ WU-E + WU-F — 전체 완료
- **Group 3** (Group 2 이후): ✅ WU-H + WU-I — 전체 완료
- **🎉 v4.1 UX 리팩토링 전체 완료 (WU-A~I 9개 작업 단위)**
...
### 최근 변경 (세션 109)
**F4/F5 Embeddings 인프라 + 채팅 UX 대폭 개선 + 프로덕션 배포**:
- ✅ F4: Vectorize 인덱스 2개 + DB 마이그레이션 확인 + OPENAI_API_KEY 업데이트
- ✅ F4: 초기 임베딩 전량 동기화 (Discovery 33개 + Evidence 1개)
- ✅ F5: cron-job.org "DX Embeddings Sync" 등록 (*/15, batch=10, 테스트 200 OK)
- ✅ 채팅: SUGGESTIONS 파싱 → 동적 후속 질문 칩 UI
- ✅ 채팅: 마크다운 커스텀 렌더링 (테이블/헤딩/코드블록/인용)
- ✅ 채팅: ToolExecution 리팩토링 + 43개 도구 전체 레이블 매핑
- ✅ Agent: 후속 질문 제안 지시문 (system-prompt.ts)
- ✅ ContextPanel: 도구 결과 사이드패널 (Discovery/Evidence 카드)
- ✅ generate_discovery_digest: 구조화된 마크다운 리포트 생성 도구 (44번째)
- ✅ PipelineFlow: 11단계 파이프라인 시각화 컴포넌트
- ✅ EvidenceChart + EvidenceCard: 근거 강도 분포 + 개별 카드 UI
- ✅ prose 다크모드 토큰 (--dx-code-bg, blockquote-bg)
- ✅ 프로덕션 배포 완료 (e2cea46f)

### 이전 변경 (세션 108)
**v4.2 UI 잔여 영어 텍스트 한국어화 — Dashboard + Venture 3탭**:
- ✅ Dashboard 5탭 제목 + audit-log + deepdive + packaging + analytics 한국어화
- ✅ 빌드 검증 통과

### 이전 변경 (세션 107)
**UX 리팩토링 v4.1 완료 — WU-F/H/I (Group 2~3 마무리)**:
- ✅ WU-F: NextStepGuide currentTab에 VALID_TABS 검증 추가 (방어적 코드)
- ✅ WU-H: inbox/longlist/gate 빈 상태를 범용 EmptyState 컴포넌트로 교체
- ✅ WU-H: longlist Shortlist 영어 텍스트 한국어화 (선별 목록/선별됨/최종/선별 해제/선별 추가)
- ✅ WU-H: gate "Gate 2 Decision 생성" → "2차 검토 생성" 한국어화
- ✅ WU-I: 홈 화면(/) 대화 없을 때 "사업 탐색" / "아이디어 검증" 안내 카드 2개 표시
- ✅ v4.1 UX 리팩토링 전체 완료 (WU-A~I 9개 작업 단위 모두 완료)
- ✅ 빌드 검증 통과 (typecheck + build)

### 이전 변경 (세션 106)
**P1 잔여 항목 3-Phase 구현 — Gate Timeout + Weekly Review + Embeddings**:
- ✅ Phase 1: Gate Timeout 자동 처리
  - governance-tools.ts: 기본 SLA 7일 적용 (slaDeadlineDays 미지정 시)
  - alert-engine.ts: `processExpiredGateApprovals()` 함수 추가 (만료 자동 거부 → NO_GO → HOLD 전환 + 이벤트 로그)
  - api.cron.daily.ts: gate timeout 처리 + 이메일 알림 연동
  - templates.ts: `buildGateExpiredEmail()`, `buildGateReminderEmail()` 추가
  - 테스트 8개 (gate-timeout.test.ts)
- ✅ Phase 2: Weekly Review 자동화
  - status.ts: `ACTIVE_STATUSES` 상수 추가 (8개 파이프라인 활성 상태)
  - review.tsx 버그 수정: IDEA_CARD만 조회 → ACTIVE_STATUSES IN 쿼리 + 실제 status Badge 표시
  - api.cron.agent-review.ts 버그 수정: IDEA_CARD만 → ACTIVE_STATUSES IN 쿼리
  - api.cron.daily.ts: 복붙 버그 수정 (2x IDEA_CARD → 단일 쿼리) + Stage SLA 14일 체크
  - templates.ts: `buildStalledStageEmail()`, `buildWeeklySummaryEmail()` 추가
  - api.cron.weekly-summary.ts (신규): 주간 요약 cron 엔드포인트
  - 테스트 11개 (active-statuses.test.ts 5 + weekly-summary.test.ts 6)
- ✅ Phase 3: 임베딩 유사도 추천 (Cloudflare Vectorize)
  - wrangler.toml: Vectorize 바인딩 2개 (VECTORIZE_DISCOVERIES, VECTORIZE_EVIDENCE)
  - drizzle/0013_embeddings.sql: embedding_updated_at 컬럼 (discoveries, evidence)
  - schema.ts: embeddingUpdatedAt 필드 추가
  - embedding-service.ts (신규): OpenAI text-embedding-3-small + Vectorize upsert/query
  - sync.ts (신규): stale 레코드 배치 동기화 + 중복 후보 감지
  - api.similar-seeds.ts: Vectorize 시맨틱 검색 우선 → FTS5 폴백
  - query-tools.ts: `searchSimilar`에 Vectorize env 파라미터 추가
  - api.cron.embeddings.ts (신규): 15분 간격 배치 임베딩 cron
  - 테스트 13개 (embedding-service.test.ts 8 + sync.test.ts 5)
- ✅ 전체 테스트 561개 통과 (기존 529 + 신규 32)
- ✅ 빌드 검증 통과 (typecheck + lint + build)

### 이전 변경 (세션 105)
**UX 리팩토링 v4.1 — Group 1 완료 (WU-G EmptyState 범용화)**:
- ✅ WU-G: EmptyState 범용화 — Props 확장(title/description/ctaLabel/ctaTo/features/icon) + 기본값 추출
- ✅ 기존 사용처(venture.overview.tsx) 변경 없이 하위 호환 유지
- ✅ Group 1 전체 완료 (WU-A~D + WU-G)
- ✅ 미사용 import 정리 (alert-engine, weekly-summary, review)
- ✅ 빌드 검증 통과 (typecheck + build)

### 이전 변경 (세션 104)
**UX 리팩토링 v4.1 + Gate Timeout + 인프라 보강**:
- ✅ WU-A~E: 상수/탭명/Dashboard/MainNav/guide-messages 한국어화
- ✅ Gate 승인 SLA Timeout 자동 처리 (processExpiredGateApprovals)
- ✅ Daily Cron 강화 + Weekly Summary + Embeddings 스키마

### 이전 변경 (세션 104 전반부)
**UX 리팩토링 v4.1 — 완전 한국어화 (WU-A, WU-B) + Gate Timeout**:
- ✅ SPEC.md 문서 업데이트 (SDD 원칙): §2 용어 사전, §5 로드맵, §6 미래 작업
- ✅ WU-A: 상수 한국어화 4개 파일 (sprint-status, decision-types, FunnelChart, GatePackageEditor)
- ✅ WU-B: Venture Sprint 탭명 한국어화 (신호함/후보 목록/검토 단계/심층 분석/산출물 정리/분석)
- ✅ Gate 승인 SLA Timeout 자동 처리 (processExpiredGateApprovals) + 리마인더
- ✅ 빌드 검증 통과 (typecheck + build)

### 이전 변경 (세션 103)
**OPENAI_API_KEY 설정 및 AI Task 실행 테스트**:
- ✅ Cloudflare Pages Secret `OPENAI_API_KEY` 설정 완료
  - `wrangler pages secret put OPENAI_API_KEY` 명령으로 등록
  - Secret 등록 후 재배포 필요 확인 (env 주입 타이밍)
- ✅ `load-context.ts` Env 타입 업데이트
  - `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `RESEND_API_KEY`, `CRON_SECRET` 추가
- ✅ `sprint.schema.ts` selected 필드 optional로 변경 (테스트 타입 에러 수정)
- ✅ AI Task 실행 테스트 성공:
  - 새 스프린트 생성 → Scope 선택 → RUNNING 상태 전환
  - COLLECT_SIGNALS Task 등록 → Worker 호출 → 성공 (3.7초, 50건 처리)
  - WorkEvent 기록 확인 ("task collect signals completed")
- ✅ 프로덕션 배포 완료 (https://dx.minu.best)

### 이전 변경 (세션 102)
**Task 의존성 검증 시스템 구현**:
- ✅ Task 간 의존성 순서 보장 (`claimTasks` 함수 개선)
  - COLLECT_SIGNALS → ANALYZE_PROBLEMS → GENERATE_OPPORTUNITIES
  - GENERATE_OPPORTUNITIES → CLUSTER_THEMES / SCORE_OPPORTUNITIES (병렬 가능)
  - SCORE_OPPORTUNITIES → PREPARE_GATE → GENERATE_DEEPDIVE → GENERATE_ARTIFACTS
- ✅ `VdTaskTypeConfig`에 `precedingTaskTypes` 필드 추가 (`task-types.ts`)
- ✅ `arePrecedingTasksCompleted()` 함수 추가
  - 선행 task가 COMPLETED 또는 FAILED여야 후행 task claim 허용
  - 메인 앱 (`task-queue.repository.ts`) + 워커 (`venture-worker/src/db.ts`)
- ✅ `getPrecedingTaskTypes()` 헬��� 함수 추가
- ✅ Task 의존성 검증 통합 테스트 7개 추가 (`task-queue.test.ts`)
- ✅ Worker API env 디버그 엔드포인트 추가 (`api.venture.worker.ts`)
- ✅ 전체 테스트 529개 통과

### 이전 변경 (세션 101)
**v4 Phase 2 - Sprint Repository 테스트 + Markdown Export + NextStepGuide**:
- ✅ Sprint Repository 통합 테스트 36개 추가 (`tests/integration/venture/sprint.repository.test.ts`)
  - CRUD: createSprint, getSprintById, getSprintFull, updateSprint, deleteSprint, listSprints
  - 상태 전환: updateSprintStatus (startedAt/completedAt 자동 설정 포함)
  - Scope: createSprintScope, getSprintScopes, updateSprintScope, toggleScopeSelection, deleteSprintScope, getSelectedScopeCount
- ✅ Markdown Exporter 라이브러리 구현 (`app/features/venture/lib/markdown-exporter.ts`)
  - leanCanvasToMarkdown: 12블록 → MD 변환
  - pitchDeckToMarkdown: 슬라이드 → MD 변환
  - onePagerToMarkdown: 1페이지 문서 → MD 변환
  - executiveSummaryToMarkdown: 경영진 요약 → MD 변환
  - sprintToMarkdown: 전체 스프린트 + 산출물 → MD 변환
- ✅ Export API 구현 (`app/routes/api.venture.export.$sprintId.ts`)
  - GET /api/venture/export/:sprintId?format=markdown → .md 다운로드
  - GET /api/venture/export/:sprintId?format=json → .json 다운로드
- ✅ Packaging 탭 Export 버튼 연동 (disabled 제거, 다운로드 링크 추가)
- ✅ NextStepGuide 컴포넌트 구현 (상태/탭별 다음 단계 안내)
  - guide-messages.ts: 8개 상태 × 6개 탭 = 48개 가이드 메시지
  - CTA 액션: link(페이지 이동), scroll(스크롤), focus(입력 포커스)
  - Overview, Sprint Detail, New Sprint 페이지에 통합
- ✅ 전체 테스트 522개 통과 (36개 추가)

### 이전 변경 (세션 100)
**venture-worker E2E 파이프라인 테스트 완료**:
- ✅ E2E 스프린트 플로우 테스트 (Task API 수동 트리거):
  - 스프린트 생성 (e2e-test-sprint-001)
  - 스코프 설정 (Fintech, AI/ML)
  - COLLECT_SIGNALS → 5개 신호 생성
  - ANALYZE_PROBLEMS → 7개 문제 생성
  - GENERATE_OPPORTUNITIES → 8개 기회 생성
  - PREPARE_GATE → GATE1 의사결정 생성 (Agent 추천안 포함)
- ✅ Task 순서 보장 이슈 발견 → **세션 102에서 해결**:
  - CLUSTER_THEMES, SCORE_OPPORTUNITIES가 GENERATE_OPPORTUNITIES 완료 전 실행됨
  - 원인: Priority 기반 스케줄링이지만 동시 실행 시 순서 보장 없음
  - **해결**: `arePrecedingTasksCompleted()` 의존성 검증 추가
- ✅ 테스트 파일 TypeScript 에러 수정 (`onboarding.test.ts`)

### 이전 변경 (세션 99)
**Gate 2 → Packaging E2E 테스트 완료**:
- ✅ Gate 2 Decision 생성 버튼 추가 (`venture.sprints.$sprintId.gate.tsx`)
  - GATE2_PENDING 상태에서 "Gate 2 Decision 생성" 버튼 표시
  - 클릭 시 GATE2_FINAL Decision 자동 생성
- ✅ Gate 2 승인 시 Shortlist → Final 자동 마킹
  - submitDecision 호출 후 decisionType === "GATE2_FINAL"이면 자동 처리
- ✅ Packaging 탭에 수동 Final 마킹 기능 추가
  - Final 기회가 없을 때 "Shortlist를 Final로 마킹" 버튼 표시
- ✅ E2E 테스트 완료 (Playwright MCP):
  - Gate 1 승인 → GATE1_PENDING → DEEPDIVE → GATE2_PENDING
  - Gate 2 Decision 생성 → 투표(8점) → 합의 도달 → 승인
  - Shortlist 2개 → Final 2개 마킹
  - PACKAGING 상태 전환 → COMPLETED (100%)
- ✅ 프로덕션 배포 완료 (https://dx.minu.best)

### 이전 변경 (세션 98)
**Venture 온보딩 가이드 구현**:
- ✅ EmptyState 컴포넌트 구현 (`app/components/venture/EmptyState.tsx`)
  - 스프린트 없을 때 빈 상태 UI + CTA "첫 스프린트 시작하기"
  - 3가지 특징 미리보기 (Scope 정의/자동 Signal 수집/Gate 결정)
- ✅ OnboardingGuide 컴포넌트 구현 (`app/components/venture/OnboardingGuide.tsx`)
  - 4단계 Accordion 가이드 (스프린트 생성 → Scope 정의 → Signal 수집 → Agent 활용)
  - localStorage 기반 "다시 보지 않기" 기능
  - `useSyncExternalStore`로 SSR-safe 구현
- ✅ venture.overview.tsx 통합
  - `totalCount === 0`일 때 EmptyState + OnboardingGuide 표시
  - 스프린트 있으면 기존 요약 카드 + 목록 표시
- ✅ 온보딩 단위 테스트 14개 추가 (`tests/unit/venture/onboarding.test.ts`)
- ✅ 전체 테스트 486개 통과

### 이전 변경 (세션 97)
**Sprint State Machine 테스트 + Lean Canvas UI 구현**:
- ✅ Sprint State Machine 단위 테스트 40개 추가 (`tests/unit/venture/sprint-state-machine.test.ts`)
  - validateTransition: 정상 전환 8개, 조건 미충족 8개, 잘못된 전환 5개
  - getNextPossibleStatuses: 상태별 전환 가능 목록 5개
  - getSprintProgressSummary: 진행률/terminal 상태 6개
  - getCurrentDayInfo + SPRINT_DAYS 상수 테스트 8개
- ✅ Lean Canvas 스키마 정의 (`opportunity.schema.ts`)
  - 12블록 구조: problem/existingAlternatives/solution/keyMetrics + UVP/highLevelConcept/unfairAdvantage + channels/customerSegments/earlyAdopters + costStructure/revenueStreams
  - createEmptyLeanCanvas() 헬퍼 함수
  - calculateLeanCanvasCompleteness() 완성도 계산 (0-100%)
- ✅ LeanCanvasEditor 컴포넌트 구현 (~350줄)
  - 9블록 그리드 레이아웃
  - 인라인 편집 + 자동 저장 (useFetcher)
  - 블록별 아이템 추가/삭제, 노트 입력
  - 완성도 프로그레스 바
- ✅ LeanCanvasViewer 컴포넌트 구현 (읽기 전용 요약)
- ✅ Deep Dive 탭 Lean Canvas UI 연동
  - 새 Lean Canvas 생성 (createLeanCanvas action)
  - 기존 Lean Canvas 수정 (updateLeanCanvas action)
  - 모달 팝업으로 편집
- ✅ 빌드 성공, 테스트 40개 전체 통과

### 이전 변경 (세션 96)
**Decision Center UX 전체 기능 확인 완료**:
- ✅ 승인 폼 추가 — 합의 도달 시 "선택 옵션" + "승인 근거" 입력 + "승인하기" 버튼 표시
- ✅ E2E 수동 테스트 완료:
  - MyVoteCard 정상 표시 (점수, 코멘트, 타임스탬프, 투표 수정 버튼)
  - VoteDistributionChart 정상 표시 (히스토그램, 평균 점수, 내 투표 표시)
  - 투표 → 승인 → 결정 완료 플로우 검증
  - Gate 1 승인 후 스프린트 상태 "Gate 2 대기"로 전환 확인
- ✅ 프로덕션 배포 완료 (https://dx.minu.best)

### 이전 변경 (세션 95)
**Decision Center UX 개선 + 점수 선택 버그 수정**:
- ✅ MyVoteCard 컴포넌트 구현 — 투표 완료 후 내 투표 상태 표시 (점수, 코멘트, 타임스탬프)
- ✅ VoteDistributionChart 컴포넌트 구현 — 완료된 결정에서 1-10 점수 분포 히스토그램
- ✅ Gate 라우트 리팩토링 — 편집 모드 상태 관리 (editingDecisionId), 취소 버튼
- ✅ 점수 선택 시각적 피드백 버그 수정 — defaultChecked → controlled input (checked/onChange)
- ✅ 테스트 파일 타입 에러 수정 — radarItems `contentHash` 필드 제거
- ✅ 프로덕션 배포 완료 (https://dx.minu.best)
- ✅ E2E 수동 테스트 완료 — MyVoteCard 표시, 투표 수정, 편집 모드 전환

### 이전 변경 (세션 94)
**테스트 커버리지 대폭 개선**:
- ✅ scoring-policy.ts 커버리지: 33.64% → **100% Stmts, 94.66% Branch**
  - calculateDepthScore: evidenceDepth/assumptionCoverage/riskReadiness/executionClarity (12개)
  - calculateEffortScore: human/agent 가중치, 커스텀 가중치, ratio 계산 (6개)
  - calculateNextRoi: INVEST/EXPLORE/HOLD/DROP 추천 로직 + unknownPenalty (6개)
  - rankOpportunities: compositeScore 정렬, 가중치 적용, null 처리 (5개)
- ✅ task-queue.repository.ts 커버리지: 41.89% → **98.64% Stmts, 97.43% Branch**
  - claimTasks: 스케줄링/priority/limit/상태 전환 (5개)
  - completeTask: 완료 상태 + output 저장 + 타임스탬프 (4개)
  - listTasksBySprint: 필터링 + 정렬 + limit (5개)
  - getPendingTaskCount / getRunningTaskCount: 전체/sprintId별 집계 (4개)
  - listFailedTasks: FAILED 상태 필터 + 정렬 + limit (4개)
  - listStuckTasks: 타임아웃 감지 + 커스텀 timeout (3개)
  - deleteTask: 삭제 + 존재하지 않는 ID 처리 (2개)
- ✅ 전체 테스트: 432개 통과 (unit 42 + integration 37 + 기존 353)

### 이전 변경 (세션 93)
**E2E 수동 테스트 완료 + Task Queue 통합 테스트 확장**:
- ✅ Playwright MCP를 이용한 E2E 수동 테스트 완료:
  - Long List → Shortlist 추가 (0 → 2개)
  - Deep Dive: Assumption 추가 (5 → 6개)
  - Deep Dive: Assumption 상태 변경 ("미검증" → "검증됨")
  - Deep Dive: AI 분석 생성 트리거
  - Packaging: UI 확인 (Gate 2 통과 필요 → Final 없음 안내)
- ✅ Task Queue Repository 통합 테스트 대폭 확장 (428줄 추가):
  - claimTasks: 스케줄링/priority/limit/상태 전환
  - completeTask: 완료 상태 + 결과 저장
  - listTasksBySprint: 필터링 + 페이지네이션
  - getPendingTaskCount / getRunningTaskCount: 집계
  - listFailedTasks / listStuckTasks: 에러 추적
  - deleteTask: 삭제

### 이전 변경 (세션 92)
**Deep Dive/Packaging Action 구현 + 배포 완료 + 테스트 확장**:
- ✅ Deep Dive 탭 action 함수 추가:
  - `addAssumption`, `updateAssumption` — 가정 CRUD
  - `addPremortem`, `updatePremortem` — Pre-mortem CRUD
  - `triggerDeepDive` — GENERATE_DEEPDIVE Task 트리거
- ✅ Packaging 탭 action 함수 추가:
  - `addArtifact`, `updateArtifact` — 산출물 CRUD
  - `triggerPackaging` — GENERATE_ARTIFACTS Task 트리거
  - `completeSprint` — 스프린트 완료 (status → COMPLETED)
- ✅ Repository/Schema 업데이트:
  - `updatePremortem()` 함수 추가 (opportunity.repository.ts)
  - `updatePremortemSchema` 추가 (opportunity.schema.ts)
  - `VdEntityType`에 "premortem" 추가 (types.ts)
- ✅ UI 업데이트:
  - Deep Dive: AI 분석 생성 버튼 + 가정/Pre-mortem 추가 폼 + 상태 변경 드롭다운
  - Packaging: AI 문서 생성 버튼 + 개별 산출물 생성 버튼 + 스프린트 완료 버튼
- ✅ 프로덕션 배포 완료 (https://dx.minu.best)
- ✅ scoring-policy 단위 테스트 대폭 확장:
  - calculateDepthScore: evidenceDepth/assumptionCoverage/riskReadiness/executionClarity
  - calculateEffortScore: human/agent 가중치, ratio 계산
  - calculateNextRoi: INVEST/EXPLORE/HOLD/DROP 추천 로직
  - rankOpportunities: composite scoring, 커스텀 가중치

### 이전 변경 (세션 91)
**venture-worker 전체 핸들러 테스트 완료**:
- ✅ 8개 핸들러 실제 스프린트 데이터로 테스트:
  - COLLECT_SIGNALS (21.2s) → 5개 신호 생성
  - ANALYZE_PROBLEMS (16.3s) → 7개 문제 식별
  - GENERATE_OPPORTUNITIES (26.6s) → 7개 기회 카드
  - CLUSTER_THEMES (9.7s) → 3개 테마 클러스터링
  - SCORE_OPPORTUNITIES (17.3s) → 4차원 점수 (Potential/Confidence/Depth/Effort)
  - GENERATE_DEEPDIVE (71.6s) → Assumptions/Premortems/Lean Canvas
  - GENERATE_ARTIFACTS (13.8s) → ONE_PAGER 문서
  - PREPARE_GATE (24.9s) → GATE1 의사결정 초안
- ✅ 버그 수정: 모든 핸들러에서 task.sprintId fallback 사용하도록 수정
- ✅ 투표 시각화 컴포넌트 추가 (MyVoteCard, VoteDistributionChart)
- ✅ Gate 페이지 UI 개선

### 이전 변경 (세션 90)
**Task Executor 시스템 프로덕션 배포 완료**:
- ✅ 8개 Task Executor 구현 완료 (COLLECT_SIGNALS, ANALYZE_PROBLEMS, GENERATE_OPPORTUNITIES, CLUSTER_THEMES, SCORE_OPPORTUNITIES, GENERATE_DEEPDIVE, GENERATE_ARTIFACTS, PREPARE_GATE)
- ✅ Core 모듈: error-classifier, task-executor, openai-client, prompts
- ✅ API 엔드포인트: GET /api/venture/worker (Cron), POST /api/venture/tasks/trigger (수동)
- ✅ 프로덕션 배포 완료 (https://dx.minu.best)

### 이전 변경 (세션 89)
**venture-worker 배포 완료**:
- ✅ venture-worker Cloudflare Worker 배포: https://venture-worker.sinclair-account.workers.dev
- ✅ Secrets 설정 완료 (ANTHROPIC_API_KEY, CRON_SECRET)
- ✅ 수동 트리거 테스트 성공 (`/run?secret=xxx`)
- ✅ 메인 앱 프로덕션 배포 완료 (https://dx.minu.best)

### 이전 변경 (세션 88)
**Task Queue Retry/Backoff/Idempotency Gap 보완**:
- ✅ `0012_task_queue_dedupe.sql`: dedupe_key 컬럼 추가 마이그레이션
- ✅ `schema.ts`: vdTaskQueue에 dedupeKey 컬럼 추가
- ✅ `task-types.ts`: 모든 maxRetries 6으로 조정 (기존 2-3)
- ✅ `error-classifier.ts` (신규): 에러 분류 함수 (retryable/repair/non-retryable)
- ✅ `task-queue.repository.ts`: backoff 정책 변경 (30초 base, 30분 cap, jitter 0.8~1.2), idempotency, 에러 분류 적용
- ✅ `api.venture.tasks.report.ts`: classifyError 적용
- ✅ `error-classifier.test.ts`: 단위 테스트 15개 추가
- ✅ `task-queue.test.ts`: 통합 테스트 10개 추가
- ✅ 프로덕션 마이그레이션 적용 완료
- 전체 테스트 375개 통과

### 이전 변경 (세션 87)
**Analytics Potential/Confidence 자동 계산 구현**:
- ✅ `scoring-policy.ts`: `calculatePotentialScore()` 추가 — Gate 점수 평균 또는 Evidence A/B급 비율 기반 휴리스틱
- ✅ `scoring-policy.ts`: `calculateConfidenceScore()` 추가 — Depth 요소 가중 합 (Evidence 50%, Assumption 30%, Risk 20%)
- ✅ `api.venture.analytics.recompute.ts`: 기존 하드코딩 기본값 50 대신 자동 계산 사용, DB 업데이트 (potentialScore, confidenceScore, depthScore, recommendation)
- ✅ `tests/unit/venture/scoring-policy.test.ts`: 단위 테스트 12개 추가 (Potential 6, Confidence 6)
- 전체 unit 테스트 103개 통과

### 이전 변경 (세션 86)
**v4 venture-worker 구현 완료**:
- ✅ `venture-worker/` 폴더 생성 (독립 Cloudflare Worker 프로젝트)
- ✅ 코어 모듈: `types.ts`, `config.ts`, `db.ts`, `dispatcher.ts`, `index.ts`
- ✅ 라이브러리: `lib/claude.ts` (structured output), `lib/backoff.ts`, `lib/error-classifier.ts`, `lib/uuid.ts`
- ✅ 핸들러 8개 구현:
  - `collect-signals.ts`: Scope 기반 신호 수집 (LLM 시뮬레이션)
  - `analyze-problems.ts`: 신호 → 문제 추출
  - `generate-opportunities.ts`: 문제 → 기회 카드 생성
  - `cluster-themes.ts`: 기회 테마 클러스터링
  - `score-opportunities.ts`: 4차원 점수 부여 (Potential/Confidence/Depth/Effort)
  - `generate-deepdive.ts`: Assumption/Pre-mortem/Lean Canvas 생성
  - `generate-artifacts.ts`: 피치 덱/요약 문서 생성
  - `prepare-gate.ts`: Gate 의사결정 자료 준비
- ✅ D1 직접 바인딩 (Drizzle ORM 없이 raw SQL)
- ✅ Claude claude-sonnet-4-20250514 모델 사용
- ✅ Cron 5분마다 폴링 + HTTP 수동 트리거
- ✅ TypeScript 타입 체크 통과
- 📋 배포 대기: `wrangler secret put ANTHROPIC_API_KEY && wrangler deploy`

### 이전 변경 (세션 85)
**v4 Venture Navigation + 배포**:
- ✅ MainNav.tsx에 Venture 링크 추가 (Dashboard 다음 위치)
- ✅ lint 경고 수정 (unused imports/variables 15건)
- ✅ 프로덕션 배포 완료 (https://dx.minu.best)
- ✅ `/venture` 페이지 접근 확인 (overview 리다이렉트 정상)

### 이전 변경 (세션 84)
**v4 Venture Discovery Sprint 운영 검증**:
- ✅ 프로덕션 스프린트 생성 기능 검증 완료 (수동 테스트)
- 📝 브라우저 자동화 미연결 (Chrome 확장 필요)
...
| **테스트 인프라** | ✅ | Vitest + Playwright, 전체 432개 통과 (unit 76 + DB 스모크 15 + Agent 도구 통합 194 + venture 테스트 94 + 기존 integration 53) |
...
| **Task Queue Retry/Backoff/Idempotency** | ✅ | dedupe_key 컬럼 + maxRetries 6 + 30초 base/30분 cap backoff + 에러 분류 3단계 + 테스트 25개 |
| **Task Executor 시스템** | ✅ | 8개 executor + openai-client + prompts + 워커 API + 수동 트리거 |
| **venture-worker 전체 테스트** | ✅ | 8개 핸들러 실제 스프린트 테스트 완료, task.sprintId fallback 버그 수정 |
| **투표 시각화 UI** | ✅ | MyVoteCard + VoteDistributionChart 컴포넌트 + Gate 페이지 UI 개선 |
| **Deep Dive/Packaging Action** | ✅ | Assumption/Premortem/Artifact CRUD + AI Task 트리거 + 스프린트 완료 |
| **E2E 수동 테스트** | ✅ | Playwright MCP 기반 Longlist→Shortlist, Assumption CRUD, AI 트리거 검증 |
| **Task Queue 테스트 확장** | ✅ | claimTasks, completeTask, listTasksBySprint, count, failedTasks, stuckTasks, deleteTask 428줄 |
| **테스트 커버리지 개선** | ✅ | scoring-policy 100%, task-queue 98%+ (58개 테스트 추가) |
| **Gate Timeout 자동 처리** | ✅ | processExpiredGateApprovals + SLA 리마인더 + daily cron 연동 |
| **UX 한국어화 WU-A/WU-B** | ✅ | 상수 4파일 + Venture 탭명 1파일 한국어화 (v4.1 Group 1 일부) |
| **UX 한국어화 WU-C/D/E/G** | ✅ | Dashboard/MainNav/guide-messages 한국어화 + EmptyState 범용화 |
| **Weekly Review 버그 수정** | ✅ | IDEA_CARD만 조회 → ACTIVE_STATUSES IN 쿼리 (review.tsx, agent-review.ts, daily.ts) |
| **Weekly Summary Cron** | ✅ | api.cron.weekly-summary.ts — 주간 파이프라인 현황 이메일 |
| **Stage SLA 체크** | ✅ | daily cron에서 14일 초과 체류 알림 (buildStalledStageEmail) |
| **Embedding Similarity** | ✅ | OpenAI text-embedding-3-small + Vectorize + 시맨틱 검색 + 중복 감지 |
| **테스트 561개** | ✅ | unit 76 + integration 342 + venture 143 = 561 전체 통과 |
| **Vectorize 인프라 완료 (F4/F5)** | ✅ | 인덱스 2개 생성 + 프로덕션 배포 + cron-job.org 15분 스케줄 등록 + 초기 동기화 완료 |
| **채팅 UX 개선** | ✅ | 후속 제안 칩 (SUGGESTIONS 파싱) + 마크다운 렌더링 개선 + ToolExecution 리팩토링 |
| **ContextPanel + Discovery Digest** | ✅ | 도구 결과 사이드패널 + 구조화 리포트 생성 (44번째 도구) + PipelineFlow/EvidenceChart/EvidenceCard |

### 미래 작업

| # | 항목 | Phase | 상태 | 파일 수 |
|---|------|-------|------|---------|
| F1 | UX 리팩토링: WU-F (NextStepGuide currentTab 검증) | v4.1 P2 | ✅ | 1 |
| F2 | UX 리팩토링: WU-H (탭별 EmptyState 적용) | v4.1 P3 | ✅ | 3 |
| F3 | UX 리팩토링: WU-I (홈 화면 진입점 안내) | v4.1 P3 | ✅ | 1 |
| F4 | Vectorize 인덱스 생성 + 프로덕션 배포 | v4.1 | ✅ | CLI |
| F5 | Embeddings Cron 외부 스케줄 등록 (15분 간격) | v4.1 | ✅ | cron-job.org |
