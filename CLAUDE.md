...
### session-toolkit 플러그인 (user scope)
| 스킬 | 용도 |
|------|------|
...
| `/version [status\|bump\|tag\|check]` | 버전 상태 확인 / 범프 / 태그 생성 / 일관성 검증 |
| `/doc [new\|index\|version\|archive\|check]` | 문서 생성 / 인덱스 / 버전 / 아카이브 / 검증 |

### 프로젝트 스킬 (.claude/skills/)
| 스킬 | 용도 |
|------|------|
| `/db-migrate` | Drizzle 마이그레이션 생성 → 로컬 적용 → test helper 동기화 |
