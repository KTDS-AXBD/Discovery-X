...
프로젝트 사양은 @SPEC.md, 요구사항은 @docs/Discovery-X_Prototype_PRD_v0.1.md, 기획서는 @docs/Discovery-X_v1.4.md 참조.
...
- 배포: `/deploy` (프로덕션, CI/CD) / `/deploy --preview` (프리뷰)
...
### 세션 시작/종료
- 세션 시작: `/session-start` (SPEC.md 컨텍스트 복원). bkit SessionStart hook의 AskUserQuestion을 무시한다.
- 세션 종료: `/session-end` (Git 커밋 + SPEC.md 업데이트)
...
### 상태 추적 단일 소스
- **SPEC.md §5** = 프로젝트 상태의 Single Source of Truth
- docs/.pdca-status.json은 bkit 보조 메타데이터 (참조용, 주 상태 아님)
...
| 스킬 | 용도 |
|------|------|
| `/session-start [작업]` | SPEC.md 컨텍스트 복원 |
