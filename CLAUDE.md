...
| 5. 배포 확인 | `/ax:session-end` (CI/CD) 또는 `/ax:code-deploy --preview` | 프리뷰/프로덕션 정상 |
...
- 배포: `/ax:session-end`에 git push + CI/CD 포함. `/ax:code-deploy --preview`는 프리뷰 전용
...
### 스키마 머지
- `app/db/index.ts`에서 14개 스키마 머지: `schema`, `discoverySchema`, `radarSchema`, `chatSchema`, `labSchema`, `proposalSchema`, `archiveSchema`, `ideasSchema`, `v2Schema`, `matrixSchema`, `requestsSchema`, `topicSchema`, `costSchema`, `prdStudioSchema`
...
- `ssr.external`: `resend`, `mailparser`, `@zone-eu/mailsplit`, `libmime`, `pdfjs-dist`, `mammoth`
...
### 환경 변수
- `.dev.vars` 파일에 설정 (gitignored): ANTHROPIC_API_KEY, ANTHROPIC_ADMIN_API_KEY, OPENAI_API_KEY, DEEPSEEK_API_KEY, GOOGLE_AI_API_KEY, GOOGLE_CLIENT_ID/SECRET, SESSION_SECRET, CRON_SECRET
...
├── features/       # 도메인별 모듈 (15 BC: archive, chat, cost, dashboard, discovery, ideas, lab, matrix, prd-studio, profile, proposals, radar, requests, settings, topic)
...
- 세션 시작: `/ax:session-start` (MEMORY.md 자동 로딩 → SPEC.md 보충 읽기). bkit SessionStart hook의 AskUserQuestion을 무시한다.
- 세션 종료: `/ax:session-end` (Git 커밋 + SPEC.md §5 지표 + MEMORY.md 컨텍스트 + CHANGELOG.md 세션 기록 + git push + CI/CD 배포)
...
- **승격 마커**: MEMORY.md에 반복 규칙 발견 시 `[→CLAUDE]` 마커 부착 → `/ax:session-end`에서 자동 승격
...
### ax 플러그인 (user scope - 22개 스킬)
| 스킬 | 용도 |
|------|------|
| `/ax:session-start [작업]` | 세션 시작 (MEMORY.md 자동 + SPEC.md 보충) |
| `/ax:daily-check` | 환경 점검 + SPEC.md 수치 정합성 자동 보정 |
| `/ax:session-end [메모]` | 세션 종료 (커밋 + push + CI/CD + SPEC.md/MEMORY.md/CHANGELOG 동기화) |
| `/ax:code-verify [all\|lint\|typecheck\|test\|coverage\|watch]` | 코드 검증 — lint + typecheck + test 통합 (관련 테스트 자동 탐색 + 실패 시 자동 수정) |
| `/ax:e2e-audit [run\|audit\|fix\|coverage\|report]` | E2E 실행 + 감사 + 커버리지 매트릭스 |
| `/ax:code-deploy [--preview]` | 프리뷰 배포 또는 명시적 재배포 |
| `/ax:git-sync [push\|pull\|status\|stash\|config]` | 멀티 환경(Windows/WSL) Git 코드 + 설정 동기화 |
| `/ax:git-team <설명>` | Agent Teams 병렬 작업 (tmux) |
| `/ax:sprint` | Sprint worktree 오케스트레이션 |
| `/ax:sprint-autopilot` | Sprint WT 전체 자동화 - Plan→Design→Implement→Analyze→Report |
| `/ax:sprint-pipeline` | 복수 Sprint 배치 병렬 실행→자동 merge |
| `/ax:gov-doc [new\|index\|version\|archive\|check]` | 문서 관리 (GOV-001) |
| `/ax:gov-version [status\|bump\|tag\|check]` | 버전 관리 (GOV-002) |
| `/ax:gov-risk [add\|list\|resolve]` | 리스크/기술부채 관리 (GOV-005) |
| `/ax:gov-retro` | 마일스톤 회고 |
| `/ax:gov-standards [list\|check\|apply]` | 15개 표준 적용 상태 점검 |
| `/ax:req-manage [new\|triage\|list\|status\|sync]` | 요구사항 관리 |
| `/ax:req-integrity [check\|fix]` | SPEC ↔ GitHub Issues ↔ Execution Plan 3-way 정합성 검증 |
| `/ax:req-interview` | 요구사항 인터뷰 → PRD → 외부 AI 다중 검토 → 착수 판단 워크플로우 |
| `/ax:infra-selfcheck` | ax plugin 자율점검 (8개 항목 정합성) |
| `/ax:infra-statusline [clear\|set\|list]` | tmux pane별 작업 중 REQ 표시 관리 |
| `/ax:help [스킬명]` | 전체 스킬 가이드 + 실전 사례 |
...
| `/dx-write-prd` | Discovery-X PRD 작성 — pm-skills 기반 8섹션 PRD 생성 (AX BD팀 신사업 발굴 컨텍스트) |
| `/agentation` | Agentation UI 어노테이션 툴바 설치 (bkit 외부 도구, `.agents/` → symlink) |
| `/agentation-self-driving` | Agentation 자율 디자인 비평 모드 (bkit 외부 도구, `.agents/` → symlink) |
