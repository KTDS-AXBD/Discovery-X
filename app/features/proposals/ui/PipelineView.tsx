  ...
  return (
    <div className="rounded-xl border border-line bg-surface-card p-4">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="text-sm font-semibold text-fg">파이프라인</h3>
      ...
      <div className="flex gap-3 overflow-x-auto pb-2">
        {PIPELINE_ORDER.map((status) => {
          ...
          return (
            <div
              key={status}
              className="min-w-[180px] flex-1 shrink-0 rounded-lg border border-line bg-surface"
            >
              {/* Column header */}
              <Link
                to={tabPath}
                className="flex items-center gap-1.5 border-b border-line px-3 py-2 transition-colors hover:bg-surface-secondary"
              >
                <span className="text-sm">{STAGE_ICONS[status]}</span>
                <span className="text-xs font-semibold text-fg">
                  {PROPOSAL_STATUS_LABELS[status]}
                </span>
                <span className="ml-auto text-xs font-bold text-fg-tertiary">
                  {count}건
                </span>
              ...
              <div className="p-2 space-y-1">
                {visibleItems.length === 0 && (
                  <p className="px-1 py-2 text-center text-[10px] text-fg-tertiary">
                    항목 없음
                  </p>
                )}
                {visibleItems.map((item) => (
                  <Link
                    ...
                    className="block truncate rounded px-2 py-1 text-xs text-fg-secondary transition-colors hover:bg-surface-secondary hover:text-fg"
                  >
                    {item.title}
                  </Link>
                ))}
                {remaining > 0 && (
                  <Link
                    to={tabPath}
                    className="block px-2 py-1 text-[10px] text-fg-tertiary hover:text-fg-brand"
                  >
                    외 {remaining}건 &rarr;
                  </Link>
                )}
