...
describe("mapModel", () => {
  ...
  it("maps sonnet 4 to google gemini-2.5-flash", () => {
    expect(mapModel("claude-sonnet-4-20250514", "google")).toBe("gemini-2.5-flash");
  });
  ...
  it("maps haiku to google gemini-2.5-flash-lite", () => {
    expect(mapModel("claude-haiku-4-5-20251001", "google")).toBe("gemini-2.5-flash-lite");
  });

  it("maps unknown model to defaults", () => {
    expect(mapModel("claude-unknown-version", "openai")).toBe("gpt-4o");
    expect(mapModel("claude-unknown-version", "google")).toBe("gemini-2.5-flash");
  });
