...
test.describe("실험실 페이지", () => {
  ...
  test.skip("실험실 5탭 네비게이션 확인", async ({ page }) => {
    // requires auth — lab.tsx 레이아웃에 5개 탭 렌더링
    await page.goto("/lab");
    await page.waitForLoadState("networkidle");
  ...
  test.skip("실험실 탭 클릭 시 올바른 URL 이동", async ({ page }) => {
    // requires auth
    await page.goto("/lab");
    await page.waitForLoadState("networkidle");
  ...
  test.skip("실험실 헤더 텍스트 확인", async ({ page }) => {
    // requires auth
    await page.goto("/lab");
    await page.waitForLoadState("networkidle");

    // lab.tsx에 "실험실" h1 + "Knowledge Graph Intelligence Laboratory" 부제
    await expect(page.getByRole("heading", { name: /실험실/i })).toBeVisible();
    await expect(page.getByText(/Knowledge Graph Intelligence Laboratory/i)).toBeVisible();
  });
  ...
  test.skip("매트릭스 — 히트맵 그리드 존재 확인", async ({ page }) => {
    // requires auth — HeatmapGrid 컴포넌트 렌더링
    await page.goto("/lab/matrix");
    await page.waitForLoadState("networkidle");
