...
export function parseMarkdown(markdown) {
    ...
    const blocks = [];
    const tables = [];
    let currentBlock = { bullets: [], subBullets: {} };
    let firstParagraph = "";
    let lastHeading = "";
    // 마크다운 테이블 감지 + 파싱
    let tableBuffer = [];
    ...
    for (const rawLine of lines) {
        ...
        if (subListMatch) {
            ...
            if (parentIdx >= 0) {
                ...
                currentBlock.subBullets[parentIdx].push(subListMatch[2].replace(/\*\*/g, "").trim());
            }
            continue;
        }
        // 리스트 아이템
        ...
        if (trimmed.length > 10 &&
            ...
            for (const sentence of sentences) {
                ...
                currentBlock.bullets.push(cleaned.length > 120 ? cleaned.slice(0, 117) + "..." : cleaned);
            }
        }
    }
    if (currentBlock.bullets.length > 0 || currentBlock.heading) {
    ...
    const keyInsight = firstParagraph?.split(/[.!?]\s/)?.[0]?.replace(/\*\*/g, "").trim() ||
        blocks[0]?.bullets[0] ||
        "";
    return { keyInsight, blocks, tables };
}
/** 파싱된 블록을 슬라이드용 불릿 목록으로 변환 (최대 maxPerSlide개) */
export function flattenBlocks(blocks, maxPerSlide = 7) {
    const pages = [];
    let current = {
        bullets: [],
        subBullets: {},
