...
export function buildSlides(data, format) {
    const slides = [];
    let order = 1;
    ...
    for (const group of SECTION_GROUPS) {
        const hasContent = group.types.some((t) => template.includes(t) && data.sections[t]?.trim());
        if (hasContent) {
    ...
    const metricBullets = [];
    if (data.budget)
        metricBullets.push(`예산 규모: ${data.budget}`);
    if (data.teamSize)
        metricBullets.push(`투입 인력: ${data.teamSize}명`);
    if (data.startDate)
        metricBullets.push(`시작 시점: ${data.startDate}`);
    if (data.milestones.length > 0) {
...
//# sourceMappingURL=slide-builder.js.map